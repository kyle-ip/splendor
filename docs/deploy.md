...
4. Site URL: `https://kyle-ip.github.io/splendor/`
...
If the repository name is not `splendor`, update `base` in [`vite.config.ts`](../vite.config.ts) and ensure the router basename matches (`import.meta.env.BASE_URL` in [`src/main.tsx`](../src/main.tsx)).
